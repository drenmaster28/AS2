	...
	public void start(Stage primaryStage) {
		try {
			...
			createAccount.btnNAccount.setOnAction((event) -> {
				...
				String username = fname.substring(0, 1) + lname + birthday.substring(size - 2);
			...
			balance.enter.setOnAction((event) -> {
				if (balance.balancechoiceBox.getValue().equals("Both")) {
					balance.txtbx.clear();
			...
			deposit.clear.setOnAction((event) -> {
				deposit.txt.clear();
			});
			...
			withdraw.enter.setOnAction((event) -> {
				if (withdraw.withdrawalToggle.getSelectedToggle().equals(withdraw.w)
					...
					double total = c.getBalance() - amount;
					if (total < 0) {
						withdraw.confirmBox.clear();
						withdraw.confirmBox.appendText("Insufficient funds");
					} else {
						c.setBalance(total);
						withdraw.confirmBox.clear();
						withdraw.confirmBox.appendText("Done!");
					}
