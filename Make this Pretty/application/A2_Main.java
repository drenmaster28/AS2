...
//			String num = login.INPUT_id.getText();
//			Integer i = Integer.valueOf(num);
//			ID = i;
//			c = u.getChequing(filename).get(ID);
//			s = u.getSaving(filename).get(ID);
//			c.setUsername("bob");
//			c.setPassword("word");
//			String username = c.generateUser();
//			c.setUsername(username);
			...
			createAccount.btnNAccount.setOnAction((event) -> {
				...
				pw.println(fname+","+lname+","+birthday+","+username+","+password);
					...
					createAccount.txtA.appendText("This is your username and password, "+username+" "+password);
					createAccount.txtA.appendText("\nPress the End Session button and start a new session to login");
					pw.close();
					try {
						fw.close();
					} catch (IOException e) {
...
//			
			...
			(balance.balancechoiceBox.getValue().equals("Both")) {
				...
				Double rbal = (double) (Math.round(bal*100.0)/100.0);
